...
#Runa cooperating agent against a stationary agent
def run():
    gameSize = 16
    allEstimatedErrors = []
    allErrors = []
    ...
    for iteration in range(100):
        agent: Agent = Agent(200, 0.1, np.array([0.05,0.1, .2, .3, .4, .5, .6, .7, .8, 0.9]), gameSize)
        opponent = StationaryOpponent(np.random.uniform(-1,1),np.random.uniform(-1,1), np.random.randint(0,gameSize-1))
        ...
        for i in range(1,timesteps+1):
            ...
            if(i % 100 == 0):
                print("Iteration:",i,"Estimated attitude: ", agent.opponent.attitude, "belief:", agent.opponent.belief)
